...
public class TechnicalSupervisor_Controller : Controller
{
    ...
    public async Task<ActionResult> AssignTask(string User_Id , int Report_Id)
    {
       ...
       var affectedRows = await conn.ExecuteAsync("UPDATE [kauSupport].[dbo].[Reports] set assignedTaskTo= @assignedTaskTo, reportStatus= @reportStatus WHERE reportID= @reportID" ,
           new
           {
    ...
    public async Task<ActionResult> GetNewReportsForSupervisor()
    {
        ...
        foreach (var device in devices)
        {
            ...
            await conn.ExecuteAsync("UPDATE [kauSupport].[dbo].[Devices] SET nextPeriodicDate = @nextPeriodicDate WHERE deviceNumber = @deviceNumber" , new {nextPeriodicDate = newPeriodicMaintenanceDate , deviceNumber = Device_Number} );
            ...
            await conn.ExecuteAsync("INSERT INTO  [kauSupport].[dbo].[Reports] ( deviceNumber, serialNumber,deviceLocatedLab, reportType , problemDescription, reportedBy , reportDate) values ( @deviceNumber, @serialNumber, @deviceLocatedLab, @reportType , @problemDescription, @reportedBy ,@reportDate ) ", 
                new {
        ...
        var response = await conn.QueryAsync<Report>("select * from  [kauSupport].[dbo].[Reports] where  reportStatus =@reportStatus" ,
